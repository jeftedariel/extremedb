...
const ORDER_BY = {
  name: sql`lower(p.name) ASC, p.id ASC`, // paridad con COLLATE NOCASE del prototipo
  price_asc: sql`lp.price ASC NULLS LAST, p.id ASC`,
  price_desc: sql`lp.price DESC NULLS LAST, p.id ASC`,
  discount: sql`(lp.regular_price - lp.price) DESC NULLS LAST, p.id ASC`,
  updated: sql`lp.captured_at DESC NULLS LAST, p.id DESC`,
};
